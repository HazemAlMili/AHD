...
- `ARCHIVED`: not active public inventory. Archiving is reversible: `POST /api/v1/admin/workers/{id}/restore` (SUPER_ADMIN, ADMIN) returns an archived worker to `DRAFT` (never directly republished) and clears `archived_at`.
...
- Every non-submit button inside a form carries an explicit `type="button"` and intended submit buttons declare `type="submit"`. Root cause fixed: the admin login form's logo button was an implicit `type="submit"` and the form's first submit button, so pressing Enter in the credentials fields activated the logo's `navigate("/home")` instead of logging in — the reported "login redirects to the public homepage" defect.
- The Docker app healthcheck uses a raw PDO probe instead of a full Laravel bootstrap (`php artisan db:show`), which periodically starved the local machine's CPU and pushed login POSTs past the frontend's bounded timeout; the adapter timeout is 20s (bounded; genuine outages still surface immediately via connection errors).
- Worker restore: `POST /api/v1/admin/workers/{id}/restore` (SUPER_ADMIN, ADMIN) moves `ARCHIVED` → `DRAFT`, clears `archived_at`, rejects non-archived workers with 422, and is audited; the admin row shows a `استعادة` action for archived workers.
- Worker permanent delete: `DELETE /api/v1/admin/workers/{id}` is `SUPER_ADMIN`-only, audited with a before-snapshot, cascades `worker_skills`/`worker_media` via FK, and deletes owned `workers/`-prefixed media files from the configured disk (S3 when configured); external-URL media (no storage key) is never fetched. The admin UI gates the destructive action behind an Arabic confirmation dialog (`حذف العاملة نهائياً؟` / `لا يمكن التراجع`) and shows the control only to SUPER_ADMIN.
- Worker status display labels are centralized in `artifacts/khadematy-site/src/lib/status.ts` (Arabic labels for availability and publication states); backend enum values, validation values, and API contract values are unchanged — only display text is Arabic, in the admin table, availability select, and public surfaces.

## 21. Current QA Status

Latest QA result: **AHD ADMIN RELIABILITY PASS** (admin reliability + worker lifecycle round). The intermittent admin login defect was fully root-caused and fixed: (1) the login form's logo button was an implicit submit button that captured Enter-key submission and navigated to the public homepage — fixed with explicit button types; (2) the heavyweight Docker healthcheck starved local CPU so login POSTs occasionally exceeded the client timeout — fixed with a raw PDO healthcheck plus a 20s bounded adapter timeout; 15/15 alternating Enter/click login cycles now pass with sub-second steady-state latency. Worker lifecycle was extended per approved requirements: archive is reversible (`restore` → DRAFT, audited, role-protected) and permanent delete exists (SUPER_ADMIN-only, Arabic confirmation, FK-cascaded relations, owned-media file cleanup), both covered by Laravel feature tests (11 tests / 117 assertions). Worker status labels shown to Arabic users are centralized and fully Arabic while backend enum values remain unchanged. The approved no-CRM, WhatsApp-first MVP scope remains unchanged. The next stage is shared-hosting staging deployment; external hosting, HTTPS, and optional S3 checks remain deployment-bound.
